...
resource "azurerm_postgresql_server" "byok_postgres" {
  count = var.dbs_count
  name = can(var.postgres_dbs[count.index]["dbname_override"]) ? var.postgres_dbs[count.index]["dbname_override"] : "${var.deploy_name}-${var.region}-byok-${var.customer_name}-${count.index}"
  location                     = var.region
  ...
  create_mode                  = "Default"
  ssl_minimal_tls_version_enforced = lookup(var.postgres_dbs[count.index],"ssl_minimal_tls_version_enforced","TLS1_2")
